...
filenames <- list.files(path=dirchosen, pattern="-log.tif", full.names=TRUE)

#Now we will apply all functions to the list of files:
lapply(filenames, function(a) {
    # load file:
    img <- readTIF (a); #This opens the tiff with all 6 channels correclty
    
    chromatin <- img[ , ,5, ] #finds the chromatin
    mask <- img[ , ,6, ] #finds the mask
...
#    Rescaling so imageJ lut can read the output
    segscale= seg$class/7;
    
    writeImage (x=segscale, file=paste(dapi_filepath, "_SEG.tif", sep=""));
